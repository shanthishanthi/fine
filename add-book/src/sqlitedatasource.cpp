...
void SQLiteDataSource::createTable()
{
    ...
    std::string sqlStr = "CREATE TABLE IF NOT EXISTS cont"
...
ErrorInfo SQLiteDataSource::getContact(Contact::ContactId id, Contact& c)
{
    ...
    std::string sqlStr = "SELECT * FROM cont WHERE id=?;";
...
{
    //create sql prepared statement
    std::string sqlStr = "SELECT * FROM Cont;";
...
{
    //create sql prepared statement
    std::string sqlStr = "INSERT INTO cont VALUES("
                        "NULL,?,?,?,?,?,?);";
...
{
    //create sql prepared statement
    std::string sqlStr = "UPDATE cont SET "
                         "firstname=?, lastname=?,"
                         "phonenum=?, address=?,"
                         "email=?,"
                         "city=? WHERE id=?;";
